...
SELECT 'herp', (SELECT cd_protocol FROM main.def_protocol WHERE protocol='Visual transect'),cd_pt_ref, ROW_NUMBER() OVER (PARTITION BY cd_pt_ref  ORDER BY year) num_gp
FROM a 
JOIN main.punto_referencia USING (cd_pt_ref)
GROUP BY cd_pt_ref, name_pt_ref, year
ORDER BY cd_pt_ref, year
...
LEFT JOIN main.gp_event ge ON b.num_gp=ge.campaign_nb AND ge.cd_gp_biol='herp' AND cd_protocol= (SELECT cd_protocol FROM main.def_protocol WHERE protocol='Visual transect') AND ge.cd_pt_ref=e.cd_pt_ref
)
...
INSERT INTO main.def_var_samp_eff(var_samp_eff, var_samp_eff_spa, cd_unit, type_variable)
  VALUES
    ...
    'Number of sets',
    'Número de lances',
    (SELECT cd_unit FROM main.def_unit WHERE unit='Number of sets'),
    'int'
    ),(
    'Number of trawl nets',
    'Número de arrastres',
    (SELECT cd_unit FROM main.def_unit WHERE unit='Number of trawl nets'),
    'int'
    )
...
VALUES(
  'Thrownets',
  'Atarraya',
  (SELECT cd_var_samp_eff FROM main.def_var_samp_eff WHERE var_samp_eff='Number of sets'),
  (SELECT cd_var_samp_eff FROM main.def_var_samp_eff WHERE var_samp_eff='Elapsed time'),
  false,
  true,
  (SELECT cd_var_ind_qt FROM main.def_var_ind_qt WHERE var_qt_ind='Number of individuals'),
...
CASE
    ...
    WHEN metodo='K' THEN (SELECT cd_protocol FROM main.def_protocol WHERE protocol='Kick Sampling')
    ...
    WHEN metodo='R' THEN (SELECT cd_protocol FROM main.def_protocol WHERE protocol='Type D net')
  END cd_protocol,
  cd_pt_ref,
  c.campaign_nb
FROM a
LEFT JOIN campaign c ON c.aguas_bajas=a.aguas_bajas AND (c.anh_name=a.anh_name OR (c.anh_name IS NULL AND a.anh_name IS NULL))
LEFT JOIN main.punto_referencia USING (cd_pt_ref)
GROUP BY a.grupo, cd_pt_ref, name_pt_ref,a.año, a.aguas_bajas, metodo, c.campaign_nb
...
CASE
  ...
  c.campaign_nb
FROM a
LEFT JOIN campaign c USING (grupo,año,aguas_bajas,anh_name)
LEFT JOIN main.punto_referencia USING (cd_pt_ref)
),d AS(
SELECT b.*,cd_gp_event
FROM b
...
LEFT JOIN raw_dwc.hidrobiologico_event e USING(cd_event)
ORDER BY r.event_date, r.event_time
