...
Las curvas de Rango-Incidencia utilizan una escala log en el eje y porque las diferencias entre las abundancias de las especies más abundantes y las especies menos abundantes pueden ser demasiado fuertes en una escala clasica.
Acá el hecho de utilizar las incidencias de las especies permite limitar esa diferencias.
Por esta razón utilizaré una escala clasica, lineal en el eje y.


## Matrix total

```{r rangoIncidenciaTotal, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia, orden según las incidencias totales, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.1,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[,names(sum_incidencia)],tempo,colSums))
...
for(i in 1:nrow(sum_incidencias_tempo))
{
  points(sum_incidencias_tempo[i,],type="b",col=colList$tempo$color[i],bg=colList$tempo$color[i],pch=colList$tempo$pch[i],lty=2)
}

legend("topright",pch=c(15,colList$tempo$pch),col=c("black",colList$tempo$color),pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
```{r rangoIncidenciaTotal_zona, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia, orden según las incidencias totales, mostrando las 3 zonas de muestreo"}
par(mar = c(10.1,6.1,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix),decreasing = T)[1:nb_sp]
sum_incidencias_zona <- Reduce(rbind,by(horm_matrix[,names(sum_incidencia)],zona,colSums))

plot(1:nb_sp,sum_incidencia,ylab="Suma de las incidencias", xlab=NA,ylim=c(0,max(sum_incidencia)*1.1),pch=15,bty="n",xaxt="n",las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, pos=0,outer=T)
text(x=(1:nb_sp)+.2,y=par("usr")[3]-1,names(sum_incidencia),xpd=NA,srt=45,adj=1.1)
for(i in 1:nrow(sum_incidencias_zona))
{
  points(sum_incidencias_zona[i,],type="b",col=colList$zona$color[i],bg=colList$zona$color[i],pch=colList$zona$pch[i],lty=2)
}

legend("topright",pch=c(15,colList$zona$pch),col="black",pt.bg=c(NA,colList$zona$color),legend=c("Ambas",colList$zonas$platform),title="Zona",bty="n")
```

## Mostrando las coberturas


```{r rangoIncidenciaTotal_cobertura, fig.height=5, fig.width=9, fig.cap="Grafica Rango-Incidencia, orden según las incidencias totales, mostrando las coberturas"}
par(mar = c(10.1,6.1,1,1))
sum_incidencia <- sort(colSums(horm_matrix),decreasing = T)[1:nb_sp]
A<-by(horm_matrix[,names(sum_incidencia)],landcov,colSums)
sum_incidencias_landcov <- Reduce(rbind,A)
rownames(sum_incidencias_landcov)<-names(A[sapply(A,length)>0])

plot(1:nb_sp,sum_incidencia,ylab="Suma de las incidencias", xlab=NA,ylim=c(0,max(sum_incidencia)*1.1),pch=15,bty="n",xaxt="n",las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, pos=0,outer=T)
text(x=(1:nb_sp)+.2,y=par("usr")[3]-1,names(sum_incidencia),xpd=NA,srt=45,adj=1.1)
for(i in 1:nrow(sum_incidencias_landcov))
{
  points(sum_incidencias_landcov[i,],type="b",col=colList$landcover$color[colList$landcover$landcov_spa==rownames(sum_incidencias_landcov)[i]],bg=colList$landcov$color[colList$landcover$landcov_spa==rownames(sum_incidencias_landcov)[i]],pch=21,lty=2)
}
legend("topright",pch=c(15,rep(21,nrow(sum_incidencias_landcov))),col=c("black",colList$landcover$color[colList$landcover$landcov_spa  %in% landcov]),pt.bg=c(NA,colList$landcover$color[colList$landcover$landcov_spa  %in% landcov]),legend=c("Todas",colList$landcover$landcov_spa[colList$landcover$landcov_spa  %in% landcov]),title="Cobertura",bty="n")
```
...
```{r rangoIncidencia_bosque_abierto, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia , orden según las incidencias totales, en los bosques abiertos, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.1,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix[landcov=="Bosque Abierto",]),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[landcov=="Bosque Abierto",names(sum_incidencia)],tempo[landcov=="Bosque Abierto"],colSums))
...
```{r rangoIncidencia_bosque_denso, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia , orden según las incidencias totales, en los bosques densos, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.1,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix[landcov=="Bosque Denso",]),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[landcov=="Bosque Denso",names(sum_incidencia)],tempo[landcov=="Bosque Denso"],colSums))
...
```{r rangoIncidencia_bosque_ripario, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia , orden según las incidencias totales, en los bosques riparios, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.1,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix[landcov=="Bosque Ripario",]),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[landcov=="Bosque Ripario",names(sum_incidencia)],tempo[landcov=="Bosque Ripario"],colSums))
...
```{r rangoIncidencia_herbazales, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia , orden según las incidencias totales, en los herbazales, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix[landcov=="Herbazales",]),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[landcov=="Herbazales",names(sum_incidencia)],tempo[landcov=="Herbazales"],colSums))
...
```{r rangoIncidencia_palma, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia , orden según las incidencias totales, en los cultivos de palma, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix[landcov=="Palma",]),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[landcov=="Palma",names(sum_incidencia)],tempo[landcov=="Palma"],colSums))
...
```{r rangoIncidencia_pastos, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia , orden según las incidencias totales, en los pastos, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_incidencia <- sort(colSums(horm_matrix[landcov=="Pastos",]),decreasing = T)[1:nb_sp]
sum_incidencias_tempo <- Reduce(rbind,by(horm_matrix[landcov=="Pastos",names(sum_incidencia)],tempo[landcov=="Pastos"],colSums))
