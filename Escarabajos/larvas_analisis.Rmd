---
title: "Análisis de los datos de larvas de escarabajos"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de larvas de escarabajos, organizados en la base de datos
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/larva_",echo=T)
...
La base de datos contiene todos los datos de larvas de escarabajos en VIEWS del schema "public", se pueden importar así:
...
infoTot<-import_esca$esca_info_sampling_larva
...
```{r cache=F}
esca_matrix <- import_esca$esca_larva_matrix
infoTot <- import_esca$esca_info_sampling_larva
...
Nota: muchos de las unidades de muestreo no muestran ningunos individuos en los registros

# Curvas de diversidad

*****************

**Anotar: El número de unidades de muestreo no corresponde a la realidad de los numero de unidades de muestreo en las tablas. En particular, no se muestran todos los eventos que no tuvieron registros, volveré a hacer esas tablas para que muestran las unidades de muestreo sin registros!
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de larvas de escarabajos colectados , calculos basados en la abundancia de las especies colectadas."}
...
```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de larvas de escarabajos colectada en la temporada de aguas altas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de larvas de escarabajos colectada en la temporada de aguas bajas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de larvas de escarabajos colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de abundancia de larvas de escarabajos.
...
Acá el estrés es muy ligero (probablemente debido al tamaño reducido de la matriz), con un valor de **`r round(nmds_tot$stress,2)`** (es decir, un $`r round(nmds_tot$stress*100)`$\% de las distancias no están el mismo orden de las distancias de Bray-Curtis), y una correlación lineal mostrando un $R^2$ de 
