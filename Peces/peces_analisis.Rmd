...
data.frame(
  ...
  column_spec(c(1,4,6,8,10),width="1.6cm")
```
...
sqldf(
  "SELECT Assemblage \"Tipo cuerpo de agua\", Observed sha_observado, ROUND(Estimator,2) sha_estimado, ROUND(\"s.e.\",2) sha_estimado_se
  FROM est_todo_tipCuerAg
  WHERE Diversity='Shannon diversity'
  ORDER BY Assemblage
  ",drv="SQLite"
...
sqldf(
  "SELECT Assemblage \"Tipo cuerpo de agua\", Observed sim_observado, ROUND(Estimator,2) sim_estimado, ROUND(\"s.e.\",2) sim_estimado_se
  FROM est_todo_tipCuerAg
  WHERE Diversity='Simpson diversity'
  ORDER BY Assemblage
  ",drv="SQLite"
...
data.frame(
  ...
  sha_observado=tabEst_todo_tipCuerAg$sha_observado,
  sha_estimado=paste(tabEst_todo_tipCuerAg$sha_estimado,tabEst_todo_tipCuerAg$sha_estimado_se,sep=" ± "),
  sim_observado=tabEst_todo_tipCuerAg$sim_observado,
  sim_estimado=paste(tabEst_todo_tipCuerAg$sim_estimado,tabEst_todo_tipCuerAg$sim_estimado_se,sep=" ± ")
 ),
data.frame(
  ...
  sha_observado=tabEst_todo_tipCuerAg_t1$sha_observado,
  sha_estimado=paste(tabEst_todo_tipCuerAg_t1$sha_estimado,tabEst_todo_tipCuerAg_t1$sha_estimado_se,sep=" ± "),
  sim_observado=tabEst_todo_tipCuerAg_t1$sim_observado,
  sim_estimado=paste(tabEst_todo_tipCuerAg_t1$sim_estimado,tabEst_todo_tipCuerAg_t1$sim_estimado_se,sep=" ± ")
 ),
data.frame(
  ...
  sha_observado=tabEst_todo_tipCuerAg_t2$sha_observado,
  sha_estimado=paste(tabEst_todo_tipCuerAg_t2$sha_estimado,tabEst_todo_tipCuerAg_t2$sha_estimado_se,sep=" ± "),
  sim_observado=tabEst_todo_tipCuerAg_t2$sim_observado,
  sim_estimado=paste(tabEst_todo_tipCuerAg_t2$sim_estimado,tabEst_todo_tipCuerAg_t2$sim_estimado_se,sep=" ± ")
 )
)%>%
  kable(booktabs=T, longtable=T,col.names = c("Temporada","Tipo de cuerpo de agua","eventos","cobertura","Riqueza colectada","observado","estimado","observado","estimado","observado","estimado"),caption="Estimaciones de diversidad por tipo de cuerpo de agua (todos artes de pesca) desde la matriz de incidencia")%>%
...
sqldf(
  "SELECT Observed sha_observado, ROUND(Estimator,2) sha_estimado, ROUND(\"s.e.\",2) sha_estimado_se
  FROM est_todo_zona
  WHERE Diversity='Shannon diversity'
  ORDER BY Assemblage
  ",drv="SQLite"
...
sqldf(
  "SELECT Observed sim_observado, ROUND(Estimator,2) sim_estimado, ROUND(\"s.e.\",2) sim_estimado_se
  FROM est_todo_zona
  WHERE Diversity='Simpson diversity'
  ORDER BY Assemblage
  ",drv="SQLite"
...
data.frame(
  ...
  sha_observado=tabEst_todo_zona$sha_observado,
  sha_estimado=paste(tabEst_todo_zona$sha_estimado,tabEst_todo_zona$sha_estimado_se,sep=" ± "),
  sim_observado=tabEst_todo_zona$sim_observado,
  sim_estimado=paste(tabEst_todo_zona$sim_estimado,tabEst_todo_zona$sim_estimado_se,sep=" ± ")
 )%>%
  kable(booktabs=T, longtable=T,col.names = c("Zona","eventos","cobertura","Riqueza colectada","observado","estimado","observado","estimado","observado","estimado"),caption="Estimaciones de diversidad por zonas (todos artes de pesca) desde la matriz de incidencia")%>%
  collapse_rows(columns = 1, valign = "middle", latex_hline = "major")%>%
  add_header_above(c(" "=4, "q0 (riqueza)" = 2,"q1 (shannon)"=2,"q2 (Simpson)"=2))%>%
  landscape
```
...
```{r perfiles_espEfectivas, fig.height=7, fig.width=9,fig.cap="Perfiles de números de especies efectivas de Hill calculadas gracias al paquete iNEXT sobre las matrices de incidencia. Las barras de errores representan los intervalos de confiancia de las estimaciones de los números de Hill"}
iNEXT_tot <- iNEXT(sort(c(sum(info_todo$nb_event),colSums(matrix_todo)),decreasing=T),datatype ="incidence_freq")
effectiveTaxNbProfile(iNEXT_tot, iNEXT_categ = iNEXT_todo_tipCuerAg, colCateg = colList$aquatic_habitats$color, categLevels = colList$aquatic_habitats$habitat_spa, titleLegend = "Tipo de cuerpo de agua")
```
...
Nota: con la incidencia, no se utiliza la escala log, pero se puede modificar para utilizar el log, no dudar en pedir!

```{r}
info_todo$tipo_cuerp_agua <- factor(info_todo$tipo_cuerp_agua,colList$aquatic_habitats$habitat_spa)
m<-match(rownames(matrix_todo),table=info_todo$anh_tempo)
tipCuerAg<-info_todo$tipo_cuerp_agua[m]
info_todo$zone <- factor(info_todo$zone,colList$zonas$platform)
zonas<-info_todo$zone[m]
tempo<-factor(gsub("ANH_[0-9]{1,3}_(T[12])","\\1",rownames(matrix_todo)),levels=colList$tempo$cd_tempo,labels=colList$tempo$temporada)
rank_abundance<-list()
```


```{r rangoIncidenciaTotal, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Incidencia con todos las artes de pesca"}
rank_abundance$incidenciaTot_tempo <- rankAbundance(matrix_todo,extraLines = tempo,type="incidence",extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(11,5.5,1,8),posAxisX = -0.06,labY="Incidencia total",byTICK = 5,byLABELS=10,posNameSp=-.1,profilesOutFig = c(.8,.99,0.45,0.7))
```
...
ordihull(nmds_tot,groups = tempo,col=colList$tempo$color,lwd=4,draw="polygon",border=NA,alpha=30)
...
plot(nmds_tot,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_tot,groups = tipCuerAg,col=colList$aquatic_habitats$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_tot,groups = anh,col="grey")
...
plot(nmds_tot,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_tot,groups = zonas, col=colList$zonas$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_tot,groups = anh,col="grey")
...
ordihull(nmds_atarraya,groups = tempo, col=colList$tempo$color,lwd=4,draw="polygon",border=NA,alpha=30)
...
plot(nmds_atarraya,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_atarraya,groups = tipCuerAg, col=colList$aquatic_habitats$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_atarraya,groups = anh,col="grey")
...
plot(nmds_atarraya,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_atarraya,groups = zonas, col=colList$zonas$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_atarraya,groups = anh,col="grey")
...
ordihull(nmds_arrastre,groups = tempo, col=colList$tempo$color,lwd=4,draw="polygon",border=NA,alpha=30)
...
plot(nmds_arrastre,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_arrastre,groups = tipCuerAg, col=colList$aquatic_habitats$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_arrastre,groups = anh,col="grey")
...
plot(nmds_arrastre,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_arrastre,groups = zonas, col=colList$zonas$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_arrastre,groups = anh,col="grey")
...
ordihull(nmds_electropesca,groups = tempo, col=colList$tempo$color,lwd=4,draw="polygon",border=NA,alpha=30)
...
plot(nmds_electropesca,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_electropesca,groups = tipCuerAg, col=colList$aquatic_habitats$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_electropesca,groups = anh,col="grey")
...
plot(nmds_electropesca,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
ordihull(nmds_electropesca,groups = zonas, col=colList$zonas$color,lwd=4,draw="polygon",border=NA,alpha=30)
ordispider(nmds_electropesca,groups = anh,col="grey")
