---
title: "Análisis de los datos de aves (cameras trampa)"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de aves (cameras trampa), organizados en la base de datos
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/aves_",echo=T)
...
La base de datos contiene todos los datos de aves en VIEWS del schema "public", se pueden importar así:
...
import_catr <- lapply(listTable,dbReadTable,conn=fracking_db)
names(import_catr) <- listTable
import_catr[grep("matrix",listTable)] <- lapply(import_catr[grep("matrix",listTable)],function(x)
  {
...
infoTot<-import_catr$catr_info_sampling_bird
coordinates(infoTot) <- ~ x_coord + y_coord
...
```{r cache=F}
catr_matrix <- import_catr$catr_bird_matrix
infoTot <- import_catr$catr_info_sampling_bird
m <- match(rownames(catr_matrix), table = infoTot$event_id)
anh <- as.numeric(gsub("ANH([0-9]{1,3})_(T[123])","\\1",rownames(catr_matrix)))
tempo <- factor(gsub("ANH([0-9]{1,3})_(T[123])","\\2",rownames(catr_matrix)))
table(tempo)
...
infoTot$event_id[!infoTot$event_id %in% infoTot$event_id[m]]
```
...
dataFor_iNEXT_SU<-apply(catr_matrix,1,function(x)sort(x[x>0],decreasing=T))
...
cbind(
sqldf(
  "SELECT Assemblage event_id, Observed observado, ROUND(Estimator,2) ||' ± '||ROUND(\"s.e.\",2) estimado
  FROM est_su
...
sqldf(
  ...
  kable(booktabs=T, longtable=T,caption = "Estimaciones de diversidad por unidad de muestreo",col.names = c("event_id","coverage",rep(c("observado","estimado"),3)))%>%
...
```{r iNEXT}
(nbSU<-table(landcov))
sum_abu <- by(catr_matrix,landcov,colSums)
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectados , calculos basados en la abundancia de las especies colectadas."}
...
**Anotar: en el periodo T1 suprimimos los datos de bosque abierto que no son suficientes para obtener una estimación de riqueza**

```{r iNEXT_landcov_t1_t2_t3}
#T1
catr_matrix_t1 <- catr_matrix[tempo=="T1" & landcov != "Bosque Abierto",]
landcov_t1<-landcov[tempo=="T1" & landcov != "Bosque Abierto"]
(nbSU_t1<- table(landcov_t1))
sum_abu <- by(catr_matrix_t1,landcov_t1,colSums)
...
#T2
catr_matrix_t2 <- catr_matrix[tempo=="T2",]
landcov_t2<-landcov[tempo=="T2"]
(nbSU_t2<- table(landcov_t2))
sum_abu <- by(catr_matrix_t2,landcov_t2,colSums)
...
#T3
catr_matrix_t3 <- catr_matrix[tempo=="T3",]
landcov_t3<-landcov[tempo=="T3"]
(nbSU_t3<- table(landcov_t3))
sum_abu <- by(catr_matrix_t3,landcov_t3,colSums)
mat_landcov_t3 <- Reduce(rbind,sum_abu)
rownames(mat_landcov_t3) <- names(sum_abu)[sapply(sum_abu,length)>0]
iNEXT_landcov_t3 <- iNEXT(apply(mat_landcov_t3,1,function(x)x[x>0]),datatype = "abundance")

```


```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectada en la temporada T1, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
A<-iNEXT_ggplot_optimizado(iNEXT_landcov_t1,type=1,col=colList$landcover$color[colList$landcover$landcov_spa %in% landcov_t1],orderAssemblage = colList$landcover$landcov_spa[colList$landcover$landcov_spa %in% landcov_t1], labY='Riqueza', title="Coberturas, temporada T1")
A
```

```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectada en la temporada T2, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
A<-iNEXT_ggplot_optimizado(iNEXT_landcov_t2,type=1,col=colList$landcover$color[colList$landcover$landcov_spa %in% landcov_t2],orderAssemblage = colList$landcover$landcov_spa[colList$landcover$landcov_spa %in% landcov_t2], labY='Riqueza', title="Coberturas, temporada T2")
A
```

```{r curvaRiqueza_landcov_t3, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectada en la temporada t3, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
A<-iNEXT_ggplot_optimizado(iNEXT_landcov_t3,type=1,col=colList$landcover$color[colList$landcover$landcov_spa %in% landcov_t3],orderAssemblage = colList$landcover$landcov_spa[colList$landcover$landcov_spa %in% landcov_t3], labY='Riqueza', title="Coberturas, temporada T3")
A
```
...
tabEst_landcov$eventos<- as.numeric(table(landcov)[table(landcov)>0])
...
tabEst_landcov_t1$eventos<- as.numeric(table(landcov_t1)[table(landcov_t1)>0])
...
tabEst_landcov_t2$eventos<- as.numeric(table(landcov_t2)[table(landcov_t2)>0])
...
est_landcov_t3 <-iNEXT_landcov_t3$AsyEst
tabEst_landcov_t3 <-
cbind(
sqldf(
  "SELECT Assemblage \"Cobertura\", Observed riq_observado, ROUND(Estimator,2) riq_estimado, ROUND(\"s.e.\",2) riq_estimado_se
...
tabEst_landcov_t3$eventos<- as.numeric(table(landcov_t3)[table(landcov_t3)>0])
tabEst_landcov_t3$completude<-(tabEst_landcov_t3$riq_observado/tabEst_landcov_t3$riq_estimado)*100
tabEst_landcov_t3$coverage <- iNEXT_landcov_t3$DataInfo$SC[order(iNEXT_landcov_t3$DataInfo$Assemblage)]
...
data.frame(
  temporada='T3',
  tipCurAg=tabEst_landcov_t3$`Cobertura`,
  eventos=tabEst_landcov_t3$eventos,
  coverage=tabEst_landcov_t3$coverage,
  comp=paste(round(tabEst_landcov_t3$completude,2),"%"),
  riq_observado=tabEst_landcov_t3$riq_observado,
  riq_estimado=paste(tabEst_landcov_t3$riq_estimado,tabEst_landcov_t3$riq_estimado_se,sep=" ± "),
  sha_observado=tabEst_landcov_t3$sha_observado,
  sha_estimado=paste(tabEst_landcov_t3$sha_estimado,tabEst_landcov_t3$sha_estimado_se,sep=" ± "),
  sim_observado=tabEst_landcov_t3$sim_observado,
  sim_estimado=paste(tabEst_landcov_t3$sim_estimado,tabEst_landcov_t3$sim_estimado_se,sep=" ± ")
 )
)%>%
...
(nbSU<-as.numeric(table(zona)))
sum_abu <- by(catr_matrix,zona,colSums)
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
tabEst_zona$eventos<- as.numeric(table( zona))
...
iNEXT_tot <- iNEXT(sort(colSums(catr_matrix),decreasing=T),datatype = "abundance")
...
rango_abundancia$total_tempo <- rankAbundance(catr_matrix,type="abundance",extraLines = tempo, legendTitle = "Temporada",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1, byTICK = 500, byLABELS = 1000,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
rango_abundancia$total_zona <- rankAbundance(catr_matrix,type="abundance",extraLines = zona,extraPch = colList$zonas$pch, extraCol = colList$zonas$color,legendTitle = "Zona",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1,byTICK = 250,byLABELS = 500,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
rango_abundancia$total_cobertura <- rankAbundance(catr_matrix,type="abundance",extraLines = landcov, extraCol = colList$landcover$color,legendTitle = "Cobertura",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1,byTICK = 250, byLABELS = 500)
```
...
mat<-catr_matrix[landcov=="Bosque Ripario",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$bosqueRipario_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Ripario"], legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),posNameSp = -.1,byTICK = 100,byLABELS = 200,profilesOutFig = c(.8,.99,0.4,0.65))
```
\clearpage
...
mat<-catr_matrix[landcov=="Bosque Denso",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$bosque_denso_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Denso"],legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),byTICK = 10, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-catr_matrix[landcov=="Herbazales",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$herbazales_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Herbazales"],legendTitle = "Temporada",legendOut = T, MARG=c(9.1,7.5,1,8),byTICK = 25, byLABELS = 50,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-catr_matrix[landcov=="Pastos",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$pastos_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Pastos"],legendTitle = "Temporada",legendOut = T, MARG=c(8,5.5,1,8),byTICK = 10, byLABELS = 20 , posNameSp = -.01,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-catr_matrix[landcov=="Palma",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$palma_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Palma"], legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),byTICK = 25, byLABELS = 50,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-catr_matrix[landcov=="Vias",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$vía_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Vias"],legendTitle = "Temporada",legendOut = T, MARG=c(9.2,6,1,8),byTICK = 5, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
Note: varias unidades de muestreo solo tienen una especie, lo que dificulta mucho el calculo de los indices de diversidad $\beta$.
Para evitar los problemas, vamos a suprimir estas unidades de muestreo en el análisis de NMDS.

```{r}
nb_sp <- apply(catr_matrix,1,function(x)sum(as.logical(x)))
sum(nb_sp==1)
anh <- anh[nb_sp>1]
tempo <- tempo[nb_sp>1]
zona <- zona[nb_sp>1]
landcov <- landcov[nb_sp>1]
catr_matrix <- catr_matrix[nb_sp>1,]
catr_matrix <- catr_matrix[,as.logical(colSums(catr_matrix))]
```
...
```{r nmds_tot_}
set.seed(212)# in order to be able to obtain the same results in two different runs
require(vegan)
(nmds_tot <- metaMDS(catr_matrix,distance='bray',trymax = 5000,trace=F,k=2))
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de abundancia de aves.
...
Acá el estrés es reducido, con un valor de **`r round(nmds_tot$stress,2)`** (es decir, un $`r round(nmds_tot$stress*100)`$\% de las distancias no están el mismo orden de las distancias de Bray-Curtis), y una correlación lineal mostrando un $R^2$ de 
...
legend("topright",pch=21:23,legend=levels(tempo),title="temporada",bty="n")
legend("topleft",fill=colList$landcover$color[levels(landcov)%in%landcov],legend=colList$landcover$landcov_spa[levels(landcov)%in%landcov],title="Cobertura",bty="n")
...
```{r nmds_tot_event_id,fig.height=15,fig.width=15}
...
ordispider(nmds_tot,groups = tempo,col=rainbow(3),lwd=4)
ordihull(nmds_tot,groups = tempo,col=colList$tempo$color,lwd=4,draw="polygon",border=NA,alpha=25)
legend("topleft",fill=rainbow(nlevels(tempo)),legend=levels(tempo), title="Temporada",cex=2)
```
...
```{r nmds_tot_spider_cobertura,fig.height=15,fig.width=15, fig.cap="Representación de las coberturas en el espacio de la NMDS. Las lineas de color gris conectan los sitios (ANH) muestreados en las tres temporadas."}
plot(nmds_tot,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
...
```{r nmds_tot_spider_zonas,fig.height=15,fig.width=15, fig.cap="Representación de las zonas en el espacio de la NMDS. Las lineas de color gris conectan los sitios (ANH) muestreados en las tres temporadas."}
plot(nmds_tot,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
