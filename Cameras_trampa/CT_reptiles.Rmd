---
title: "Análisis de los datos de reptiles (cameras trampa)"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de reptiles (cameras trampa), organizados en la base de datos
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/reptiles_",echo=T)
...
La base de datos contiene todos los datos de reptiles en VIEWS del schema "public", se pueden importar así:
...
infoTot<-import_catr$catr_info_sampling_reptile
...
```{r cache=F}
catr_matrix <- import_catr$catr_reptile_matrix
infoTot <- import_catr$catr_info_sampling_reptile
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectados , calculos basados en la abundancia de las especies colectadas."}
...
**ANOTAR:en el periodo T1 no se consideran los datos de pastos que no son suficientes para obtener una estimación de riqueza**

**idem para los bosques abiertos de T2**

```{r iNEXT_landcov_t1_t2_t3}
#T1
catr_matrix_t1 <- catr_matrix[tempo=="T1" & landcov!="Pastos",]
landcov_t1<-landcov[tempo=="T1" & landcov!="Pastos"]
...
#T2
catr_matrix_t2 <- catr_matrix[tempo=="T2" & landcov != "Bosque Abierto",]
landcov_t2<-landcov[tempo=="T2"& landcov != "Bosque Abierto"]
...
```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectada en la temporada T1, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectada en la temporada T2, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t3, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectada en la temporada t3, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
rango_abundancia$total_tempo <- rankAbundance(catr_matrix,type="abundance",extraLines = tempo, legendTitle = "Temporada",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1, byTICK = 20, byLABELS = 40,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
rango_abundancia$total_zona <- rankAbundance(catr_matrix,type="abundance",extraLines = zona,extraPch = colList$zonas$pch, extraCol = colList$zonas$color,legendTitle = "Zona",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1,byTICK = 50,byLABELS = 100,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
rango_abundancia$total_cobertura <- rankAbundance(catr_matrix,type="abundance",extraLines = landcov, extraCol = colList$landcover$color,legendTitle = "Cobertura",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1,byTICK = 25, byLABELS = 50)
```
...
rango_abundancia$bosqueRipario_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Ripario"], legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),posNameSp = -.1,byTICK = 25,byLABELS = 50,profilesOutFig = c(.8,.99,0.4,0.65))
```
\clearpage
...
Solo hay datos en T3

```{r rangoAbundanciaTotal_Bosque_Denso, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Abundancia, orden según las abundancias totales, en las bosque densos, mostrando las temporadas ."}
mat<-catr_matrix[landcov=="Bosque Denso",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$bosque_denso_tempo <- rankAbundance(mat, type="abundance",legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),byTICK = 10, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$pastos_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Pastos"],legendTitle = "Temporada",legendOut = T, MARG=c(8,5.5,1,8),byTICK = 5, byLABELS = 10 , posNameSp = -.01,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$palma_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Palma"], legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),byTICK = 5, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
save_in_excel("reptiles_dataRangoAbundancia.xlsx",rango_abundancia)
```
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de abundancia de reptiles.
...
Acá el estrés es reducido (pero lo poco datos que hay no permite hacer conclusiones), con un valor de **`r round(nmds_tot$stress,2)`** (es decir, un $`r round(nmds_tot$stress*100)`$\% de las distancias no están el mismo orden de las distancias de Bray-Curtis), y una correlación lineal mostrando un $R^2$ de 
...
legend("topright",fill=rainbow(nlevels(tempo)),legend=levels(tempo), title="Temporada",cex=2)
```
