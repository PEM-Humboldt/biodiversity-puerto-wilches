---
title: "Análisis de los datos de mamiferos (cameras trampa)"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de mamiferos (cameras trampa), organizados en la base de datos
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/mamiferos_",echo=T)
...
La base de datos contiene todos los datos de mamiferos en VIEWS del schema "public", se pueden importar así:
...
infoTot<-import_catr$catr_info_sampling_mammal
...
```{r cache=F}
catr_matrix <- import_catr$catr_mammal_matrix
infoTot <- import_catr$catr_info_sampling_mammal
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamiferos colectados , calculos basados en la abundancia de las especies colectadas."}
...
#T1
catr_matrix_t1 <- catr_matrix[tempo=="T1",]
landcov_t1<-landcov[tempo=="T1"]
...
```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamiferos colectada en la temporada T1, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamiferos colectada en la temporada T2, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t3, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamiferos colectada en la temporada t3, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamiferos colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
rango_abundancia$total_tempo <- rankAbundance(catr_matrix,type="abundance",extraLines = tempo, legendTitle = "Temporada",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1, byTICK = 1000, byLABELS = 1000,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$pastos_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Pastos"],legendTitle = "Temporada",legendOut = T, MARG=c(8,5.5,1,8),byTICK = 1, byLABELS = 1 , posNameSp = -.01,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
save_in_excel("mamiferos_dataRangoAbundancia.xlsx",rango_abundancia)
```
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de abundancia de mamiferos.
