...
import_fipl[grep("matrix",listTable)] <- lapply(import_fipl[grep("matrix",listTable)],function(x)
  {
...
lapply(paste("../bioenv",dir("../bioenv/",pattern="fipl.*RData"),sep="/"),load,envir=.GlobalEnv)
```
## Resultados
...
```{r}
require(vegan)
(RDA_fipl <- rda(formula=transf~.,data=envir[varBest]))
scoreRda<-scores(RDA_fipl)
```
...
```{r}
eig <- c(RDA_fipl$CCA$eig,RDA_fipl$CA$eig)
rsq <- RsquareAdj(RDA_fipl)$adj.r.squared
```
...
barplot(eig,col=c(rep("red",length(RDA_fipl$CCA$eig)),rep("grey",length(RDA_fipl$CA$eig))),names.arg = NA, xlab=NA,ylab="Valores propios")
legend("topright",fill=c("red","grey"),legend=c("constrained","unconstrained"))
par(xpd=T)
text(length(RDA_fipl$CA$eig)/2,-0.004,"Ejes")
```

Proporción de inercia de los ejes "constrained":

```{r}
(p_EC <- RDA_fipl$CCA$eig/sum(eig))
...
```{r}
info_todo <- import_fipl$fipl_info_sampling
...
points(RDA_fipl,display="cn",col="blue")
text(RDA_fipl,display="cn",col="blue")
```
...
plot(RDA_fipl,display=c("species"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_fipl,display="cn",col="blue")
text(RDA_fipl,display="cn",col="blue")
text(RDA_fipl,display="species",cex=.5)
```
...
plot(RDA_fipl,display=c("sites","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_fipl,display="cn",col="blue")
text(RDA_fipl,display="cn",col="blue")
text(RDA_fipl,display="sites",cex=.5)
```
...
ordiplot(RDA_fipl,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_fipl,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color)
ordiellipse(RDA_fipl,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_fipl,tipCuerAg,col=colList$aquatic_habitats$color,lty=3)
points(RDA_fipl,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_fipl,display="cn",col="red",head.arrow = .1)
text(RDA_fipl,display="cn",col="red")
...
ordiplot(RDA_fipl,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_fipl,zonas,kind="sd",conf=0.95,col=colList$zonas$color)
ordiellipse(RDA_fipl,zonas,kind="sd",conf=0.95,col=colList$zonas$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_fipl,zonas,col=colList$zonas$color,lty=3)
points(RDA_fipl,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_fipl,display="cn",col="red",head.arrow = .1)
text(RDA_fipl,display="cn",col="red")
...
ordiplot(RDA_fipl,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_fipl,tempo,kind="sd",conf=0.95,col=colList$tempo$color)
ordiellipse(RDA_fipl,tempo,kind="sd",conf=0.95,col=colList$tempo$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_fipl,tempo,col=colList$tempo$color,lty=3)
points(RDA_fipl,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_fipl,display="cn",col="black",head.arrow = .1)
text(RDA_fipl,display="cn",col="black")
