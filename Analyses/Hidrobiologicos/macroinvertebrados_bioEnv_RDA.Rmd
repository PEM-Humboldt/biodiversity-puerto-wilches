...
import_minv[grep("matrix",listTable)] <- lapply(import_minv[grep("matrix",listTable)],function(x)
  {
...
lapply(paste("../bioenv",dir("../bioenv/",pattern="minv.*RData"),sep="/"),load,envir=.GlobalEnv)
```
## Resultados
...
```{r}
require(vegan)
(RDA_minv <- rda(formula=transf~.,data=envir[varBest]))
scoreRda<-scores(RDA_minv)
```
...
```{r}
eig <- c(RDA_minv$CCA$eig,RDA_minv$CA$eig)
rsq <- RsquareAdj(RDA_minv)$adj.r.squared
```
...
barplot(eig,col=c(rep("red",length(RDA_minv$CCA$eig)),rep("grey",length(RDA_minv$CA$eig))),names.arg = NA, xlab=NA,ylab="Valores propios")
legend("topright",fill=c("red","grey"),legend=c("constrained","unconstrained"))
par(xpd=T)
text(length(RDA_minv$CA$eig)/2,-0.004,"Ejes")
```

Proporción de inercia de los ejes "constrained":

```{r}
(p_EC <- RDA_minv$CCA$eig/sum(eig))
...
```{r}
info_todo <- import_minv$minv_info_sampling
...
plot(RDA_minv,display=c("species"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_minv,display="cn",col="blue")
text(RDA_minv,display="cn",col="blue")
text(RDA_minv,display="species",cex=.5)
```
...
plot(RDA_minv,display=c("sites","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_minv,display="cn",col="blue")
text(RDA_minv,display="cn",col="blue")
text(RDA_minv,display="sites",cex=.5)
```
...
ordiplot(RDA_minv,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_minv,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color)
ordiellipse(RDA_minv,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_minv,tipCuerAg,col=colList$aquatic_habitats$color,lty=3)
points(RDA_minv,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_minv,display="cn",col="red",head.arrow = .1)
text(RDA_minv,display="cn",col="red")
...
ordiplot(RDA_minv,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_minv,zonas,kind="sd",conf=0.95,col=colList$zonas$color)
ordiellipse(RDA_minv,zonas,kind="sd",conf=0.95,col=colList$zonas$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_minv,zonas,col=colList$zonas$color,lty=3)
points(RDA_minv,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_minv,display="cn",col="red",head.arrow = .1)
text(RDA_minv,display="cn",col="red")
...
ordiplot(RDA_minv,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_minv,tempo,kind="sd",conf=0.95,col=colList$tempo$color)
ordiellipse(RDA_minv,tempo,kind="sd",conf=0.95,col=colList$tempo$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_minv,tempo,col=colList$tempo$color,lty=3)
points(RDA_minv,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_minv,display="cn",col="black",head.arrow = .1)
text(RDA_minv,display="cn",col="black")
