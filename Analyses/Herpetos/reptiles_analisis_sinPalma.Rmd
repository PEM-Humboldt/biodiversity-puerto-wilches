...
Se trata en este documento de presentar los análisis realizados sobre los datos de reptiles, organizados en la base de datos. En este documento, sacamos los datos que corresponden a Palma en el mapa de coberturas.
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/reptiles_sinPal_",echo=T)
...
plot(infoTot[infoTot$landcov_spa!="Palma",], bg = colList$landcover$color[match(infoTot$landcov_spa[infoTot$landcov_spa!="Palma"], table= colList$landcover$landcov_spa)],cex=2,pch=22,add=T)
...
herp_matrix<-herp_matrix[landcov!="Palma",]
m<-m[landcov!="Palma"]
landcov<-landcov[landcov!="Palma"]
...
rango_abundancia$total_cobertura <- rankAbundance(herp_matrix,type="abundance",extraLines = landcov, extraCol = colList$landcover$color,legendTitle = "Cobertura",legendOut = T, MARG=c(10,4.5,1,8),posNameSp = -.1,byTICK = 250, byLABELS = 500)
```
...
rango_abundancia$bosqueRipario_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Ripario"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9,4.5,1,8),posNameSp = -.1,byTICK = 100,byLABELS = 200,profilesOutFig = c(.8,.99,0.4,0.65))
```
\clearpage
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$bosque_abierto_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Abierto"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9.5,4.5,1,8),posNameSp = -.05,byTICK =  10, byLABELS = 20,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$herbazales_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Herbazales"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9.1,4.5,1,8),byTICK = 50, byLABELS = 100,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
save_in_excel("reptiles_dataRangoAbundancia_sinPal.xlsx",rango_abundancia)
```
