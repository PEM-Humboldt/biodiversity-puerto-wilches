---
title: "Análisis de los datos de reptiles"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de reptiles, organizados en la base de datos
...
infoTot<-import_herp$herp_info_sampling_reptiles
...
```{r cache=F}
herp_matrix <- import_herp$herp_reptiles_matrix
infoTot <- import_herp$herp_info_sampling_reptiles
...
```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectada en la temporada de aguas altas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de reptiles colectada en la temporada de aguas bajas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
rango_abundancia$total_tempo <- rankAbundance(herp_matrix,type="abundance",extraLines = tempo,extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1, byTICK = 50, byLABELS = 100,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
rango_abundancia$total_zona <- rankAbundance(herp_matrix,type="abundance",extraLines = zona,extraPch = colList$zonas$pch, extraCol = colList$zonas$color,legendTitle = "Zona",legendOut = T, MARG=c(10,6.5,1,8),posNameSp = -.1,byTICK = 50,byLABELS = 100,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
rango_abundancia$total_cobertura <- rankAbundance(herp_matrix,type="abundance",extraLines = landcov, extraCol = colList$landcover$color,legendTitle = "Cobertura",legendOut = T, MARG=c(10,4.5,1,8),posNameSp = -.1,byTICK =50, byLABELS = 100)
```
...
rango_abundancia$bosqueRipario_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Ripario"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9,4.5,1,8),posNameSp = -.1,byTICK = 50,byLABELS = 100,profilesOutFig = c(.8,.99,0.4,0.65))
```
\clearpage
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$bosque_abierto_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Abierto"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9.5,4.5,1,8),posNameSp = -.05,byTICK =  1, byLABELS = 1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$bosque_denso_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Bosque Denso"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9,6.5,1,8),byTICK = 5, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$herbazales_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Herbazales"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9.1,4.5,1,8),byTICK = 10, byLABELS = 20,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$pastos_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Pastos"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(8,6.6,1,8),byTICK = 1, byLABELS = 1,posNameSp = -0,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$vía_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Vias"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9.2,4.5,1,8),byTICK = 5, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
```{r rangoAbundanciaTotal_agua, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Abundancia, orden según las abundancias totales, en los cuerpos de aguas, mostrando las temporadas. El subgrafico \"Perfiles\" incluye todas las especies y está ordenado según las abundancias en cada temporada."}
mat<-herp_matrix[landcov=="Cuerpos Agua",]
mat<-mat[,as.logical(colSums(mat))]
rango_abundancia$vía_tempo <- rankAbundance(mat, type="abundance",extraLines = tempo[landcov=="Cuerpos Agua"], extraPch= colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(9.2,4.5,1,8),byTICK = 5, byLABELS = 10,posNameSp = -.1,profilesOutFig = c(.8,.99,0.4,0.65))
```
...
```{r}
herp_matrix["ANH_109_T2",herp_matrix["ANH_109_T2",]>0]
colSums(herp_matrix[,c("Micrurus dumerilii","Oxyrhopus petolarius")])
```

En la ANH_109_T2, solo hay 2 individuos muestreados, esta unidad de muestreo no puede estar integrada en los análisis, porque 50% de su abundancia (*Oxyrhopus petolarius*) está solo presente en este sitio

```{r nmds_tot_}
set.seed(12)# in order to be able to obtain the same results in two different runs
require(vegan)
anh <- anh[-which(rownames(herp_matrix)=="ANH_109_T2")]
landcov <- landcov[-which(rownames(herp_matrix)=="ANH_109_T2")]
zona <- zona[-which(rownames(herp_matrix)=="ANH_109_T2")]
tempo <- tempo[-which(rownames(herp_matrix)=="ANH_109_T2")]
herp_matrix <- herp_matrix[-which(rownames(herp_matrix)=="ANH_109_T2"),]
herp_matrix <- herp_matrix[,as.logical(colSums(herp_matrix))]
