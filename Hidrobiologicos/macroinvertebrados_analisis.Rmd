---
title: "Análisis de los datos de macroinvertebrados"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de macroinvertebrados, organizados en la base de datos
...
infoTot<-import_minv$minv_info_sampling
...
stopifnot(all(rownames(import_minv$minv_ab_abs_matrix)==rownames(import_minv$minv_matrix)))
infoTot <- import_minv$minv_info_sampling
m <- match(rownames(minv_matrix), table = infoTot$anh_tempo)
anh <- as.numeric(gsub("ANH_([0-9]{1,3})_(T[12])","\\1",rownames(minv_matrix)))
tempo <- factor(gsub("ANH_([0-9]{1,3})_(T[12])","\\2",rownames(minv_matrix)),levels = colList$tempo$cd_tempo, labels = colList$tempo$temporada)
table(tempo)
...
minv_matrix <- import_minv$minv_ab_abs_matrix
dataFor_iNEXT_SU<-apply(minv_matrix,1,function(x)sort(x[x>0],decreasing=T))
...
sum_abu <- by(minv_matrix,tipCuerAg,colSums)
...
```{r CurvaRiqueza_tipCuerAg_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de macroinvertebrados colectados , calculos basados en la abundancia de las especies colectadas."}
...
#T1
minv_matrix_t1 <- minv_matrix[tempo=="Aguas altas",]
tipCuerAg_t1<-tipCuerAg[tempo=="Aguas altas"]
(nbSU_t1<- table(tipCuerAg_t1))
sum_abu <- by(minv_matrix_t1,tipCuerAg_t1,colSums)
...
#T2
minv_matrix_t2 <- minv_matrix[tempo=="Aguas bajas",]
tipCuerAg_t2<-tipCuerAg[tempo=="Aguas bajas"]
(nbSU_t2<- table(tipCuerAg_t2))
sum_abu <- by(minv_matrix_t2,tipCuerAg_t2,colSums)
...
```{r curvaRiqueza_tipCuerAg_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de macroinvertebrados colectada en la temporada de aguas altas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_tipCuerAg_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de macroinvertebrados colectada en la temporada de aguas bajas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
sum_abu <- by(minv_matrix,zona,colSums)
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de macroinvertebrados colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
mean_density <- sort(colMeans(minv_matrix),decreasing = T)[1:nb_sp]
...
points(log10(colMeans(minv_matrix[grep("T1",rownames(minv_matrix)),names(mean_density)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T1"],lty=2)
points(log10(colMeans(minv_matrix[grep("T2",rownames(minv_matrix)),names(mean_density)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T2"],lty=2)
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.25,names(mean_density),xpd=NA,srt=45,adj=1.1)
points(1:nb_sp,log10(colMeans(minv_matrix[zona=="Caracterización",names(mean_density)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Caracterización"],bg=colList$zonas$color[colList$zonas$platform=="Caracterización"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Caracterización"])
points(1:nb_sp,log10(colMeans(minv_matrix[zona=="Kalé",names(mean_density)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Kalé"], bg=colList$zonas$color[colList$zonas$platform=="Kalé"], lty=2, col=colList$zonas$color[colList$zonas$platform=="Kalé"])
points(1:nb_sp,log10(colMeans(minv_matrix[zona=="Platero",names(mean_density)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Platero"],bg=colList$zonas$color[colList$zonas$platform=="Platero"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Platero"])
...
mean_density <- sort(colMeans(minv_matrix),decreasing = T)[1:nb_sp]
p_mean_densities_tipCuerAg <- by(data=minv_matrix,INDICES = tipCuerAg, FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
...
nb_sp<-15
par(mar = c(7.1,6.5,1,1))
mean_density <- sort(colMeans(minv_matrix[tipCuerAg=="Caño",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=minv_matrix[tipCuerAg=="Caño",],INDICES = tempo[tipCuerAg=="Caño"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
...
nb_sp<-15
par(mar = c(7.1,6.5,1,1))
mean_density <- sort(colMeans(minv_matrix[tipCuerAg=="Ciénaga",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=minv_matrix[tipCuerAg=="Ciénaga",],INDICES = tempo[tipCuerAg=="Ciénaga"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
mean_densities_tempo <- Reduce(rbind,p_mean_densities_tempo,)
rownames(mean_densities_tempo) <- names(p_mean_densities_tempo)[!sapply(p_mean_densities_tempo,is.null)]
plot(1:nb_sp,log10(mean_density),ylab="Densidad promedia", xlab=NA,ylim=c(0,max(log10(rbind(mean_density,mean_densities_tempo)))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
...
mean_density <- sort(colMeans(minv_matrix[tipCuerAg=="Quebrada",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=minv_matrix[tipCuerAg=="Quebrada",],INDICES = tempo[tipCuerAg=="Quebrada"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
mean_densities_tempo <- Reduce(rbind,p_mean_densities_tempo,)
rownames(mean_densities_tempo) <- names(p_mean_densities_tempo)[!sapply(p_mean_densities_tempo,is.null)]
plot(1:nb_sp,log10(mean_density),ylab="Densidad promedia", xlab=NA,ylim=c(0,max(log10(rbind(mean_density,mean_densities_tempo)))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
...
nb_sp<-15
par(mar = c(7.1,6.5,1,1))
mean_density <- sort(colMeans(minv_matrix[tipCuerAg=="Río",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=minv_matrix[tipCuerAg=="Río",],INDICES = tempo[tipCuerAg=="Río"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
mean_densities_tempo <- Reduce(rbind,p_mean_densities_tempo,)
rownames(mean_densities_tempo) <- names(p_mean_densities_tempo)[!sapply(p_mean_densities_tempo,is.null)]
plot(1:nb_sp,log10(mean_density),ylab="Densidad promedia", xlab=NA,ylim=c(0,max(log10(rbind(mean_density,mean_densities_tempo)))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
...
require(vegan)
(nmds_tot <- metaMDS(minv_matrix,distance='bray',trymax = 1000,trace=F,k=2))
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de densidad de macroinvertebrados.
