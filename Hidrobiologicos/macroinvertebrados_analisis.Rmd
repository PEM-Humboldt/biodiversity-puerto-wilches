...
# Function para las curvas de acumulación de especies 

Esa función es una adaptación de los códigos de Vladimir Sandoval.
...
iNEXT_tot <- iNEXT(sort(colSums(minv_matrix),decreasing=T),datatype ="abundance")
...
rango_densidad$total_tempo <- rankAbundance(minv_matrix,type="density",extraLines = tempo,extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(6,5.5,1,8),posAxisX = -0.06,labY="densidad promedia (ind/m2)",byTICK = 20,byLABELS=20,posNameSp=-.1)
```
...
rango_densidad$total_zona <- rankAbundance(minv_matrix,type="density",extraLines = zona,extraPch = colList$zonas$pch, extraCol = colList$zonas$color,legendTitle = "Zonas",legendOut = T, MARG=c(6,5.5,1,8),posAxisX = -0.06,labY="densidad promedia (ind/m2)",byTICK = 20,byLABELS=20, posNameSp=-.1)
```
...
rango_densidad$total_tipCuerAg <- rankAbundance(minv_matrix,type="density",extraLines = tipCuerAg, extraCol = colList$aquatic_habitats$color,legendTitle = "Tipos de cuerpo de agua",legendOut = T, MARG=c(6,5.5,1,8),posAxisX = -0.06,labY="densidad promedia (ind/m2)",byTICK = 20,byLABELS=20,posNameSp = -.1)
```
...
mat <- minv_matrix[tipCuerAg=="Caño",]
mat <- mat[,as.logical(colSums(mat))]
rango_densidad$caño_tempo <- rankAbundance(mat,type="density",extraLines = tempo[tipCuerAg=="Caño"],extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(6,5.5,1,8),posAxisX = -0.06,labY="densidad promedia (ind/m2)",byTICK = 20,byLABELS=20,posNameSp=-.1)
```
...
mat <- minv_matrix[tipCuerAg=="Ciénaga",]
mat <- mat[,as.logical(colSums(mat))]
rango_densidad$cienaga_tempo <- rankAbundance(mat,type="density",extraLines = tempo[tipCuerAg=="Ciénaga"],extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(6,5.5,1,8),posAxisX = -0.09,labY="densidad promedia (ind/m2)",byTICK = 25,byLABELS=50,posNameSp=-.1)
```
...
mat <- minv_matrix[tipCuerAg=="Quebrada",]
mat <- mat[,as.logical(colSums(mat))]
rango_densidad$quebrada_tempo <- rankAbundance(mat,type="density",extraLines = tempo[tipCuerAg=="Quebrada"],extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(6.5,6.5,1,8),posAxisX = -0.09,labY="densidad promedia (ind/m2)",byTICK = 5,byLABELS=10,posNameSp=-.1)
```
...
```{r rangoDensidadRio, fig.height=5, fig.width=8, fig.cap="Grafica Rango-densidad en las quebradas, orden según las densidades promedias totales, mostrando las temporadas. El subgrafico \"Perfiles\" incluye todas los taxones y está ordenado según las densidades en cada temporada."}
mat <- minv_matrix[tipCuerAg=="Río",]
mat <- mat[,as.logical(colSums(mat))]
rango_densidad$rio_tempo <- rankAbundance(mat,type="density",extraLines = tempo[tipCuerAg=="Río"],extraPch = colList$tempo$pch, extraCol = colList$tempo$color,legendTitle = "Temporada",legendOut = T, MARG=c(6.5,6.5,1,8),posAxisX = -0.07,labY="densidad promedia (ind/m2)",byTICK = 5,byLABELS=10,posNameSp=-.1)
```
...
save_in_excel("minv_dataRangoDensidad.xlsx",rango_densidad)
```
