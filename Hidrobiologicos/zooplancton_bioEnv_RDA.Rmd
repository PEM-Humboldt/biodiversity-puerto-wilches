---
title: "Análisis Bio-Env y RDA sobre zooplancton"
...
import_zopl[grep("matrix",listTable)] <- lapply(import_zopl[grep("matrix",listTable)],function(x)
  {
...
lapply(paste("../bioenv",dir("../bioenv/",pattern="zopl.*RData"),sep="/"),load,envir=.GlobalEnv)
```
## Resultados
...
```{r}
require(vegan)
(RDA_zopl <- rda(formula=transf~.,data=envir[varBest]))
scoreRda<-scores(RDA_zopl)
```
...
```{r}
eig <- c(RDA_zopl$CCA$eig,RDA_zopl$CA$eig)
rsq <- RsquareAdj(RDA_zopl)$adj.r.squared
```
...
barplot(eig,col=c(rep("red",length(RDA_zopl$CCA$eig)),rep("grey",length(RDA_zopl$CA$eig))),names.arg = NA, xlab=NA,ylab="Valores propios")
legend("topright",fill=c("red","grey"),legend=c("constrained","unconstrained"))
par(xpd=T)
text(length(RDA_zopl$CA$eig)/2,-0.004,"Ejes")
```

Proporción de inercia de los ejes "constrained":

```{r}
(p_EC <- RDA_zopl$CCA$eig/sum(eig))
...
```{r}
info_todo <- import_zopl$zopl_info_sampling
...
points(RDA_zopl,display="cn",col="blue")
text(RDA_zopl,display="cn",col="blue")
```
...
plot(RDA_zopl,display=c("species"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_zopl,display="cn",col="blue")
text(RDA_zopl,display="cn",col="blue")
text(RDA_zopl,display="species",cex=.5)
```
...
plot(RDA_zopl,display=c("sites","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_zopl,display="cn",col="blue")
text(RDA_zopl,display="cn",col="blue")
text(RDA_zopl,display="sites",cex=.5)
```
...
ordiplot(RDA_zopl,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_zopl,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color)
ordiellipse(RDA_zopl,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_zopl,tipCuerAg,col=colList$aquatic_habitats$color,lty=3)
points(RDA_zopl,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_zopl,display="cn",col="red",head.arrow = .1)
text(RDA_zopl,display="cn",col="red")
...
ordiplot(RDA_zopl,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_zopl,zonas,kind="sd",conf=0.95,col=colList$zonas$color)
ordiellipse(RDA_zopl,zonas,kind="sd",conf=0.95,col=colList$zonas$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_zopl,zonas,col=colList$zonas$color,lty=3)
points(RDA_zopl,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_zopl,display="cn",col="red",head.arrow = .1)
text(RDA_zopl,display="cn",col="red")
...
ordiplot(RDA_zopl,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_zopl,tempo,kind="sd",conf=0.95,col=colList$tempo$color)
ordiellipse(RDA_zopl,tempo,kind="sd",conf=0.95,col=colList$tempo$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_zopl,tempo,col=colList$tempo$color,lty=3)
points(RDA_zopl,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_zopl,display="cn",col="black",head.arrow = .1)
text(RDA_zopl,display="cn",col="black")
