---
title: "Análisis Bio-Env y RDA sobre perifiton"
...
import_peri[grep("matrix",listTable)] <- lapply(import_peri[grep("matrix",listTable)],function(x)
  {
...
lapply(paste("../bioenv",dir("../bioenv/",pattern="peri.*RData"),sep="/"),load,envir=.GlobalEnv)
```
## Resultados
...
```{r}
require(vegan)
(RDA_peri <- rda(formula=transf~.,data=envir[varBest]))
scoreRda<-scores(RDA_peri)
```
...
```{r}
eig <- c(RDA_peri$CCA$eig,RDA_peri$CA$eig)
rsq <- RsquareAdj(RDA_peri)$adj.r.squared
```
...
barplot(eig,col=c(rep("red",length(RDA_peri$CCA$eig)),rep("grey",length(RDA_peri$CA$eig))),names.arg = NA, xlab=NA,ylab="Valores propios")
legend("topright",fill=c("red","grey"),legend=c("constrained","unconstrained"))
par(xpd=T)
text(length(RDA_peri$CA$eig)/2,-0.004,"Ejes")
```

Proporción de inercia de los ejes "constrained":

```{r}
(p_EC <- RDA_peri$CCA$eig/sum(eig))
...
```{r}
info_todo <- import_peri$peri_info_sampling
...
points(RDA_peri,display="cn",col="blue")
text(RDA_peri,display="cn",col="blue")
```
...
plot(RDA_peri,display=c("species"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_peri,display="cn",col="blue")
text(RDA_peri,display="cn",col="blue")
text(RDA_peri,display="species",cex=.5)
```
...
plot(RDA_peri,display=c("sites","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
points(RDA_peri,display="cn",col="blue")
text(RDA_peri,display="cn",col="blue")
text(RDA_peri,display="sites",cex=.5)
```
...
ordiplot(RDA_peri,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_peri,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color)
ordiellipse(RDA_peri,tipCuerAg,kind="sd",conf=0.95,col=colList$aquatic_habitats$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_peri,tipCuerAg,col=colList$aquatic_habitats$color,lty=3)
points(RDA_peri,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_peri,display="cn",col="red",head.arrow = .1)
text(RDA_peri,display="cn",col="red")
...
ordiplot(RDA_peri,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_peri,zonas,kind="sd",conf=0.95,col=colList$zonas$color)
ordiellipse(RDA_peri,zonas,kind="sd",conf=0.95,col=colList$zonas$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_peri,zonas,col=colList$zonas$color,lty=3)
points(RDA_peri,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_peri,display="cn",col="red",head.arrow = .1)
text(RDA_peri,display="cn",col="red")
...
ordiplot(RDA_peri,display=c("sites","species","cn"),xlab=paste("RDA1 (",pc_EC[1],"%)",sep=""),ylab=paste("RDA2 (",pc_EC[2],"%)",sep=""),pch=NA,type="n")
ordiellipse(RDA_peri,tempo,kind="sd",conf=0.95,col=colList$tempo$color)
ordiellipse(RDA_peri,tempo,kind="sd",conf=0.95,col=colList$tempo$color,draw="polygon",alpha=50,border=NA)
ordispider(RDA_peri,tempo,col=colList$tempo$color,lty=3)
points(RDA_peri,display="sites",col=COL,bg=COL,pch=PCH,cex=.8)
points(RDA_peri,display="cn",col="black",head.arrow = .1)
text(RDA_peri,display="cn",col="black")
