...
```{r}
load("bioEnv_minv.RData")
varBest <- names(envir)[bioEnv$models[[bioEnv$whichbest]][[1]]]
```
...
require(vegan)
minv_hellinger <- decostand(minv_matrix,method = "hellinger")
(RDA_minv <- rda(minv_hellinger,envir[,varBest]))
RsquareAdj(RDA_minv)
plot(RDA_minv)
#plot(RDA,display="lc")
#text(RDA,display="species")
```
