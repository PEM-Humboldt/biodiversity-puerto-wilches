---
title: "Análisis de los datos de zooplancton"
...
Se trata en este documento de presentar los análisis realizados sobre los datos de zooplancton, organizados en la base de datos
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/zopl_",echo=T)
...
La base de datos contiene todos los datos de zooplancton en VIEWS del schema "public", se pueden importar así:
...
import_zopl <- lapply(listTable,dbReadTable,conn=fracking_db)
names(import_zopl) <- listTable
import_zopl <- c(import_zopl,lapply(import_zopl[grep("matrix",listTable)],function(x)
  {
...
names(import_zopl)[grepl("matrix",names(import_zopl)) & !sapply(lapply(import_zopl,class),function(x) "matrix"%in% x)]<- 
	paste(names(import_zopl)[grepl("matrix",names(import_zopl)) & !sapply(lapply(import_zopl,class),function(x) "matrix"%in% x)],"_raw",sep="" )
import_zopl$zopl_ab_abs_matrix<-dbTab2mat(import_zopl$zopl_matrix_raw, col_samplingUnits = "anh_tempo", col_species = "taxon", col_content = "abs_abundance")
```
...
infoTot<-import_zopl$zopl_info_sampling
...
```{r cache=F}
zopl_matrix <- import_zopl$zopl_matrix
stopifnot(all(rownames(import_zopl$zopl_ab_abs_matrix)==rownames(import_zopl$zopl_matrix)))
infoTot <- import_zopl$zopl_info_sampling
m <- match(rownames(zopl_matrix), table = infoTot$anh_tempo)
anh <- as.numeric(gsub("ANH_([0-9]{1,3})_(T[12])","\\1",rownames(zopl_matrix)))
tempo <- factor(gsub("ANH_([0-9]{1,3})_(T[12])","\\2",rownames(zopl_matrix)),levels = colList$tempo$cd_tempo, labels = colList$tempo$temporada)
table(tempo)
...
zopl_matrix <- import_zopl$zopl_ab_abs_matrix
dataFor_iNEXT_SU<-apply(zopl_matrix,1,function(x)sort(x[x>0],decreasing=T))
...
sum_abu <- by(zopl_matrix,tipCuerAg,colSums)
...
```{r CurvaRiqueza_tipCuerAg_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de zooplancton colectados , calculos basados en la abundancia de las especies colectadas."}
...
#T1
zopl_matrix_t1 <- zopl_matrix[tempo=="Aguas altas",]
tipCuerAg_t1<-tipCuerAg[tempo=="Aguas altas"]
(nbSU_t1<- table(tipCuerAg_t1))
sum_abu <- by(zopl_matrix_t1,tipCuerAg_t1,colSums)
...
#T2
zopl_matrix_t2 <- zopl_matrix[tempo=="Aguas bajas",]
tipCuerAg_t2<-tipCuerAg[tempo=="Aguas bajas"]
(nbSU_t2<- table(tipCuerAg_t2))
sum_abu <- by(zopl_matrix_t2,tipCuerAg_t2,colSums)
...
```{r curvaRiqueza_tipCuerAg_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de zooplancton colectada en la temporada de aguas altas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_tipCuerAg_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de zooplancton colectada en la temporada de aguas bajas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
sum_abu <- by(zopl_matrix,zona,colSums)
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de zooplancton colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
```{r}
zopl_matrix <- import_zopl$zopl_matrix*100
```
...
mean_density <- sort(colMeans(zopl_matrix),decreasing = T)[1:nb_sp]
...
points(log10(colMeans(zopl_matrix[grep("T1",rownames(zopl_matrix)),names(mean_density)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T1"],lty=2)
points(log10(colMeans(zopl_matrix[grep("T2",rownames(zopl_matrix)),names(mean_density)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T2"],lty=2)
...
ATX <- c(1,seq(500,1000000,by=500))
ATX <- ATX[1:(min(which(ATX>mean_density[1]))+1)]
#ATX <- ATX[(max(which(ATX<mean_density[nb_sp]))-1):min(which(ATX>mean_density[1]))+1]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.25,names(mean_density),xpd=NA,srt=45,adj=1.1)
points(1:nb_sp,log10(colMeans(zopl_matrix[zona=="Caracterización",names(mean_density)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Caracterización"],bg=colList$zonas$color[colList$zonas$platform=="Caracterización"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Caracterización"])
points(1:nb_sp,log10(colMeans(zopl_matrix[zona=="Kalé",names(mean_density)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Kalé"], bg=colList$zonas$color[colList$zonas$platform=="Kalé"], lty=2, col=colList$zonas$color[colList$zonas$platform=="Kalé"])
points(1:nb_sp,log10(colMeans(zopl_matrix[zona=="Platero",names(mean_density)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Platero"],bg=colList$zonas$color[colList$zonas$platform=="Platero"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Platero"])
...
mean_density <- sort(colMeans(zopl_matrix),decreasing = T)[1:nb_sp]
p_mean_densities_tipCuerAg <- by(data=zopl_matrix,INDICES = tipCuerAg, FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
...
nb_sp<-15
par(mar = c(7.1,6.5,1,1))
mean_density <- sort(colMeans(zopl_matrix[tipCuerAg=="Caño",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=zopl_matrix[tipCuerAg=="Caño",],INDICES = tempo[tipCuerAg=="Caño"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
...
nb_sp<-15
par(mar = c(7.1,6.5,1,1))
mean_density <- sort(colMeans(zopl_matrix[tipCuerAg=="Ciénaga",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=zopl_matrix[tipCuerAg=="Ciénaga",],INDICES = tempo[tipCuerAg=="Ciénaga"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
mean_densities_tempo <- Reduce(rbind,p_mean_densities_tempo,)
rownames(mean_densities_tempo) <- names(p_mean_densities_tempo)[!sapply(p_mean_densities_tempo,is.null)]
plot(1:nb_sp,log10(mean_density),ylab="Densidad promedia", xlab=NA,ylim=c(0,max(log10(rbind(mean_density,mean_densities_tempo)))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
...
mean_density <- sort(colMeans(zopl_matrix[tipCuerAg=="Quebrada",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=zopl_matrix[tipCuerAg=="Quebrada",],INDICES = tempo[tipCuerAg=="Quebrada"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
mean_densities_tempo <- Reduce(rbind,p_mean_densities_tempo,)
rownames(mean_densities_tempo) <- names(p_mean_densities_tempo)[!sapply(p_mean_densities_tempo,is.null)]
plot(1:nb_sp,log10(mean_density),ylab="Densidad promedia", xlab=NA,ylim=c(0,max(log10(rbind(mean_density,mean_densities_tempo)))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)

axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=-.1)
ATX <- c(1,seq(500,10000,by=500))
ATX <- ATX[1:(min(which(ATX>max(rbind(mean_density,mean_densities_tempo))))+1)]
#ATX <- ATX[(max(which(ATX<mean_density[nb_sp]))-1):min(which(ATX>mean_density[1]))+1]
ATX <- unique(ATX[c(1,which(ATX%%10==0),length(ATX))])
show <- c(T, ATX[2:length(ATX)]%%10==0)
...
nb_sp<-15
par(mar = c(7.1,6.5,1,1))
mean_density <- sort(colMeans(zopl_matrix[tipCuerAg=="Río",]),decreasing = T)[1:nb_sp]
p_mean_densities_tempo <- by(data=zopl_matrix[tipCuerAg=="Río",],INDICES = tempo[tipCuerAg=="Río"], FUN = function(x,c)colMeans(x[,c]),c=names(mean_density))
mean_densities_tempo <- Reduce(rbind,p_mean_densities_tempo,)
rownames(mean_densities_tempo) <- names(p_mean_densities_tempo)[!sapply(p_mean_densities_tempo,is.null)]
plot(1:nb_sp,log10(mean_density),ylab="Densidad promedia", xlab=NA,ylim=c(0,max(log10(rbind(mean_density,mean_densities_tempo)))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)

axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=-.05)
ATX <- c(1,seq(500,10000,by=500))
ATX <- ATX[1:(min(which(ATX>max(rbind(mean_density,mean_densities_tempo))))+1)]
#ATX <- ATX[(max(which(ATX<mean_density[nb_sp]))-1):min(which(ATX>mean_density[1]))+1]
ATX <- unique(ATX[c(1,which(ATX%%10==0),length(ATX))])
...
show[c(1,length(ATX))] <- T
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.05,names(mean_density),xpd=NA,srt=45,adj=1.1)
...
ANH_293_T1: suprimido por diferencias demasiado grandes
...
anh<-anh[rownames(zopl_matrix)!='ANH_293_T1']
tempo<-tempo[rownames(zopl_matrix)!='ANH_293_T1']
zona<-zona[rownames(zopl_matrix)!='ANH_293_T1']
tipCuerAg<-tipCuerAg[rownames(zopl_matrix)!='ANH_293_T1']
zopl_matrix <- zopl_matrix[rownames(zopl_matrix)!='ANH_293_T1',]
zopl_matrix <- zopl_matrix[,as.logical(colSums(zopl_matrix))]

require(vegan)
(nmds_tot <- metaMDS(zopl_matrix,distance='bray',trymax = 1000,trace=F,k=2))
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de densidad de zooplancton.
