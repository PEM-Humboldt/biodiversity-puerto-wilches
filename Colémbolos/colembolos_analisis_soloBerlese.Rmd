...
La idea acá es aplicar todos los análisis soló sobre los datos de Berlese, para ver si los análisis cambian en comparación con los análisis sobre la asociación entre berlese y pitfall...
...
require(iNEXT)
require(ggplot2)
knitr::opts_chunk$set(cache=T,tidy.opts = list(width.cutoff = 70), tidy = TRUE, connection="fracking_db", max.print=50,fig.path="./Fig/Berlese_",echo=T)
...
infoTot<-import_cole$cole_info_sampling_berlese
...
```{r cache=F}
cole_matrix <- import_cole$cole_berlese_matrix
infoTot <- import_cole$cole_info_sampling_berlese
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de colémbolos colectados con trasmallo, calculos basados en la abundancia de las especies colectadas."}
...
tabEst_landcov$eventos<- tapply(infoTot$nb_event[m],landcov,sum)[!is.na(tapply(infoTot$nb_event[m],landcov,sum))]
...
sum_abundancia <- sort(colSums(cole_matrix),decreasing = T)[1:nb_sp]
...
points(log10(colSums(cole_matrix[grep("T1",rownames(cole_matrix)),names(sum_abundancia)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T1"],lty=2)
points(log10(colSums(cole_matrix[grep("T2",rownames(cole_matrix)),names(sum_abundancia)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T2"],lty=2)
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(1:nb_sp,log10(colSums(cole_matrix[zona=="Caracterización",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Caracterización"],bg=colList$zonas$color[colList$zonas$platform=="Caracterización"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Caracterización"])
points(1:nb_sp,log10(colSums(cole_matrix[zona=="Kalé",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Kalé"], bg=colList$zonas$color[colList$zonas$platform=="Kalé"], lty=2, col=colList$zonas$color[colList$zonas$platform=="Kalé"])
points(1:nb_sp,log10(colSums(cole_matrix[zona=="Platero",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Platero"],bg=colList$zonas$color[colList$zonas$platform=="Platero"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Platero"])
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
sum_abundancias_cobertura <- by(data=cole_matrix,INDICES = landcov, FUN = function(x,c)colSums(x[,c]),c=names(sum_abundancia))
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(cole_matrix[landcov=="Bosque Denso",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,11000,by=5)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
#ATX <- ATX[(max(which(ATX<sum_abundancia[nb_sp]))-1):min(which(ATX>sum_abundancia[1]))+1]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(cole_matrix[landcov=="Bosque Denso" & grepl("T1",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(cole_matrix[landcov=="Bosque Denso" & grepl("T2",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(cole_matrix[landcov=="Bosque Ripario",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,11000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(cole_matrix[landcov=="Bosque Ripario" & grepl("T1",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(cole_matrix[landcov=="Bosque Ripario" & grepl("T2",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(cole_matrix[landcov=="Herbazales",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,11000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(cole_matrix[landcov=="Herbazales" & grepl("T1",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(cole_matrix[landcov=="Herbazales" & grepl("T2",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(cole_matrix[landcov=="Palma",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,11000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(cole_matrix[landcov=="Palma" & grepl("T1",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(cole_matrix[landcov=="Palma" & grepl("T2",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(cole_matrix[landcov=="Pastos",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,11000,by=1)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
#ATX <- ATX[(max(which(ATX<sum_abundancia[nb_sp]))-1):min(which(ATX>sum_abundancia[1]))+1]
ATX <- unique(ATX[c(1,which(ATX%%5==0),length(ATX))])
show <- ATX%%5==0
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(cole_matrix[landcov=="Pastos" & grepl("T1",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(cole_matrix[landcov=="Pastos" & grepl("T2",rownames(cole_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
require(vegan)
(nmds_tot <- metaMDS(cole_matrix,distance='bray',trymax = 5000,trace=F,k=2))
...
Acá el estrés es muy fuerte, con un valor de **`r round(nmds_tot$stress,2)`** (es decir, un $`r round(nmds_tot$stress*100)`$\% de las distancias no están el mismo orden de las distancias de Bray-Curtis), y una correlación lineal mostrando un $R^2$ de 
...
La NMDS **no encuentra una solución convergente**, es decir, después de intentar 5000 configuraciones aleatorias del metodo, no encuentra 2 análisis NMDS que muestran exactamente los mismos patrones... Eso es el señal de una inestabilidad de la representación de los patrones de composición en 2 dimensiones de espacio. No muestro los resultados en este documento, pero pasar a **3 dimensiones en lugar de 2** permite encontrar una solución convergente despues de 600 configuraciones aleatorias...
