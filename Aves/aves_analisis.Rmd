---
title: "Análisis de los datos de aves"
author: "avesus Bottin"
date: "`r Sys.Date()`"
lang: "es"
output: 
    pdf_document:
...
Se trata en este documento de presentar los análisis realizados sobre los datos de aves, organizados en la base de datos
...
import_aves[grep("matrix",listTable)] <- lapply(import_aves[grep("matrix",listTable)],function(x)
  {
...
tempo <- factor(gsub("ANH_([0-9]{1,3})_(T[12])","\\2",rownames(aves_matrix)),levels = colList$tempo$cd_tempo, labels = colList$tempo$temporada)
table(tempo)
...
**Anotar: para realizar el documento rápidamente, utilizo acá las abundancias tal como calculadas para hacer la matriz de datos de aves. Sin embargo es posible que el modo de calculo de las abundancias introduzca un sesgo en las estimaciones de diversidad. Tengo que tomar un tiempo de reflexión para pensarlo... En 1/2 días volveré con una decisión de como tratar esos datos en el marco de la estimación de diversidad...**
...
sqldf(
  ...
  kable(booktabs=T, longtable=T,caption = "Estimaciones de diversidad por unidad de muestreo")%>%
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectados con trasmallo, calculos basados en la abundancia de las especies colectadas."}
...
```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectada en la temporada de aguas altas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de aves colectada en la temporada de aguas bajas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
data.frame(
  ...
  kable(booktabs=T, longtable=T,col.names = c("Temporada","Cobertura","eventos","Riqueza colectada","observado","estimado","observado","estimado","observado","estimado"),caption="Estimaciones de diversidad por Cobertura desde la matriz de abundancia")%>%
...
data.frame(
  ...
  kable(booktabs=T, longtable=T,col.names = c("Zona","eventos","Riqueza colectada","observado","estimado","observado","estimado","observado","estimado"),caption="Estimaciones de diversidad por zonas desde la matriz de abundancia")%>%
  add_header_above(c(" "=3, "q0 (riqueza)" = 2,"q1 (Shannon)"=2,"q2 (Simpson)"=2))
```
...
sum_abundancia <- sort(colSums(aves_matrix),decreasing = T)[1:nb_sp]
...
points(log10(colSums(aves_matrix[grep("T1",rownames(aves_matrix)),names(sum_abundancia)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T1"],lty=2)
points(log10(colSums(aves_matrix[grep("T2",rownames(aves_matrix)),names(sum_abundancia)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T2"],lty=2)
...
```{r rangoAbundanciaTotal_zona, fig.height=5, fig.width=9, fig.cap="Grafica Rango-Abundancia, orden según las abundancias totales, mostrando las 3 zonas de muestreo"}
nb_sp<-15
par(mar = c(8.1,6.5,1,6.1))
sum_abundancia <- sort(colSums(aves_matrix),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=10)
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(1:nb_sp,log10(colSums(aves_matrix[zona=="Caracterización",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Caracterización"],bg=colList$zonas$color[colList$zonas$platform=="Caracterización"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Caracterización"])
points(1:nb_sp,log10(colSums(aves_matrix[zona=="Kalé",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Kalé"], bg=colList$zonas$color[colList$zonas$platform=="Kalé"], lty=2, col=colList$zonas$color[colList$zonas$platform=="Kalé"])
points(1:nb_sp,log10(colSums(aves_matrix[zona=="Platero",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Platero"],bg=colList$zonas$color[colList$zonas$platform=="Platero"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Platero"])
par(xpd=T)
legend("topright",pch=c(15,colList$zonas$pch),col=c("black",colList$zonas$color),pt.bg=c(NA,colList$zonas$color),legend=c("Todas",colList$zonas$platform),title="Zonas",bty="n",inset = c(-.2,-0.05))
```
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
sum_abundancias_cobertura <- by(data=aves_matrix,INDICES = landcov, FUN = function(x,c)colSums(x[,c]),c=names(sum_abundancia))
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(aves_matrix[landcov=="Bosque Abierto",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(as.matrix(aves_matrix[landcov=="Bosque Abierto" & grepl("T1",rownames(aves_matrix)), names(sum_abundancia)]))), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(aves_matrix[landcov=="Bosque Abierto" & grepl("T2",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(aves_matrix[landcov=="Bosque Denso",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=5)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(aves_matrix[landcov=="Bosque Denso" & grepl("T1",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(aves_matrix[landcov=="Bosque Denso" & grepl("T2",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(aves_matrix[landcov=="Bosque Ripario",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(aves_matrix[landcov=="Bosque Ripario" & grepl("T1",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(aves_matrix[landcov=="Bosque Ripario" & grepl("T2",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```

### Herbazales

```{r rangoAbundancia_herbazales, fig.height=5, fig.width=8, fig.cap="Grafica Rango-Abundancia , orden según las abundancias totales, en los herbazales, mostrando las temporadas"}
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(aves_matrix[landcov=="Herbazales",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(aves_matrix[landcov=="Herbazales" & grepl("T1",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(aves_matrix[landcov=="Herbazales" & grepl("T2",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(aves_matrix[landcov=="Palma",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(aves_matrix[landcov=="Palma" & grepl("T1",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(aves_matrix[landcov=="Palma" & grepl("T2",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(aves_matrix[landcov=="Pastos",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=1)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(aves_matrix[landcov=="Pastos" & grepl("T1",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(aves_matrix[landcov=="Pastos" & grepl("T2",rownames(aves_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
require(vegan)
(nmds_tot <- metaMDS(aves_matrix,distance='bray',trymax = 5000,trace=F,k=2))
...
```{r nmds_tot_spider_temporada,fig.height=15,fig.width=15, fig.cap="Representación de las temporadas en el espacio de la NMDS. Las lineas de color gris conectan los sitios (ANH) muestreados más de una vez."}
...
```{r nmds_tot_spider_cobertura,fig.height=15,fig.width=15, fig.cap="Representación de las coberturas en el espacio de la NMDS. Las lineas de color gris conectan los sitios (ANH) muestreados más de una vez."}
plot(nmds_tot,type="n",yaxt="n",xaxt="n",xlab=NA,ylab=NA)
