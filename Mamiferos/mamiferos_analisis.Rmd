---
title: "Análisis de los datos de mamíferos"
author: "mamius Bottin"
date: "`r Sys.Date()`"
lang: "es"
output: 
    pdf_document:
...
Se trata en este documento de presentar los análisis realizados sobre los datos de mamíferos, organizados en la base de datos
...
La base de datos contiene todos los datos de mamíferos en VIEWS del schema "public", se pueden importar así:
...
import_mami <- lapply(listTable,dbReadTable,conn=fracking_db)
names(import_mami) <- listTable
import_mami[grep("matrix",listTable)] <- lapply(import_mami[grep("matrix",listTable)],function(x)
  {
...
infoTot<-import_mami$mami_info_sampling_mist_nets
...
```{r cache=F}
mami_matrix <- import_mami$mami_mist_nets_matrix
infoTot <- import_mami$mami_info_sampling_mist_nets
m <- match(rownames(mami_matrix), table = infoTot$anh_tempo)
anh <- as.numeric(gsub("ANH_([0-9]{1,3})_(T[12])","\\1",rownames(mami_matrix)))
tempo <- factor(gsub("ANH_([0-9]{1,3})_(T[12])","\\2",rownames(mami_matrix)),levels = colList$tempo$cd_tempo, labels = colList$tempo$temporada)
table(tempo)
...
dataFor_iNEXT_SU<-apply(mami_matrix,1,function(x)sort(x[x>0],decreasing=T))
...
```{r iNEXT}
(nbSU<-table(landcov))
sum_abu <- by(mami_matrix,landcov,colSums)
...
```{r CurvaRiqueza_landcov_raw, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamíferos colectados con trasmallo, calculos basados en la abundancia de las especies colectadas."}
...
#T2
mami_matrix_t2 <- mami_matrix[tempo=="Aguas bajas",]
landcov_t2<-landcov[tempo=="Aguas bajas"]
(nbSU_t2<- table(landcov_t2))
sum_abu <- by(mami_matrix_t2,landcov_t2,colSums)
...
```{r curvaRiqueza_landcov_t1, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamíferos colectada en la temporada de aguas altas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
```{r curvaRiqueza_landcov_t2, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamíferos colectada en la temporada de aguas bajas, calculos basados en la abundancia total de las especies colectadas."}
 ## especificaciones de ggplot
...
sum_abu <- by(mami_matrix,zona,colSums)
...
```{r CurvaRiqueza_zona, fig.width=7, fig.height=5,fig.cap="Curvas de rarefacción/extrapolación de la riqueza de mamíferos colectada, calculos basados en la abundancia de las especies colectadas. Representación de las zonas"}
 ## especificaciones de ggplot
...
sum_abundancia <- sort(colSums(mami_matrix),decreasing = T)[1:nb_sp]
...
points(log10(colSums(mami_matrix[grep("T1",rownames(mami_matrix)),names(sum_abundancia)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T1"],lty=2)
points(log10(colSums(mami_matrix[grep("T2",rownames(mami_matrix)),names(sum_abundancia)])),type="b",pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"],bg=colList$tempo$color[colList$tempo$cd_tempo=="T2"],lty=2)
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(1:nb_sp,log10(colSums(mami_matrix[zona=="Caracterización",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Caracterización"],bg=colList$zonas$color[colList$zonas$platform=="Caracterización"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Caracterización"])
points(1:nb_sp,log10(colSums(mami_matrix[zona=="Kalé",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Kalé"], bg=colList$zonas$color[colList$zonas$platform=="Kalé"], lty=2, col=colList$zonas$color[colList$zonas$platform=="Kalé"])
points(1:nb_sp,log10(colSums(mami_matrix[zona=="Platero",names(sum_abundancia)])),type="b",pch=colList$zonas$pch[colList$zonas$platform=="Platero"],bg=colList$zonas$color[colList$zonas$platform=="Platero"],lty=2,col=colList$zonas$color[colList$zonas$platform=="Platero"])
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
sum_abundancias_cobertura <- by(data=mami_matrix,INDICES = landcov, FUN = function(x,c)colSums(x[,c]),c=names(sum_abundancia))
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(mami_matrix[landcov=="Bosque Ripario",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(mami_matrix[landcov=="Bosque Ripario" & grepl("T1",rownames(mami_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(mami_matrix[landcov=="Bosque Ripario" & grepl("T2",rownames(mami_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
nb_sp<-15
par(mar = c(10.1,6.5,1,4.1))
sum_abundancia <- sort(colSums(mami_matrix[landcov=="Palma",]),decreasing = T)[1:nb_sp]
plot(1:nb_sp,log10(sum_abundancia),ylab="Abundancia total", xlab=NA,ylim=c(0,max(log10(sum_abundancia))*1.1),pch=15,bty="n",xaxt="n",yaxt="n", las=2,type="b",lty=3)
axis(side=1,at=1:nb_sp,labels=FALSE,lwd.tick=-1, outer=T, pos=0)
ATX <- seq(0,1000,by=10)
ATX <- ATX[1:(min(which(ATX>sum_abundancia[1]))+1)]
...
axis(side=2,at=c(0,log10(ATX)[2:length(ATX)]),labels=ifelse(show,ATX,NA),las=2)
text(x=(1:nb_sp)+.2,y=-.1,names(sum_abundancia),xpd=NA,srt=45,adj=1.1)
points(log10(colSums(mami_matrix[landcov=="Palma" & grepl("T1",rownames(mami_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T1"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T1"], lty=2)
points(log10(colSums(mami_matrix[landcov=="Palma" & grepl("T2",rownames(mami_matrix)), names(sum_abundancia)])), type="b", pch=colList$tempo$pch[colList$tempo$cd_tempo=="T2"], bg = colList$tempo$color[colList$tempo$cd_tempo=="T2"], lty=2)
legend("topright",pch=c(15,colList$tempo$pch),col="black",pt.bg=c(NA,colList$tempo$color),legend=c("Ambas","Aguas altas","Aguas bajas"),title="Temporada",bty="n")
```
...
require(vegan)
(nmds_tot <- metaMDS(mami_matrix,distance='bray',trymax = 1000,trace=F,k=2))
...
En la figura \ref{ndms_tot_stressplot}, se muestra el "estrés" del método aplicado sobre la matriz de occurrencia de mamíferos.
